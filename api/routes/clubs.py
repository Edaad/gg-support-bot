...
def _club_to_read(club: Club) -> ClubRead:
    return ClubRead(
        ...
        member_join_preamble_text=club.member_join_preamble_text,
        member_join_tos_file_id=club.member_join_tos_file_id,
        member_join_tos_caption=club.member_join_tos_caption,
        ...
        cashout_soft_limit=club.cashout_soft_limit,
...
def create_club(
    ...
    if (
        db.query(ClubLinkedAccount)
        .filter_by(telegram_user_id=body.telegram_user_id)
        .first()
    ):
        raise HTTPException(
...
@router.post(
    "/{club_id}/linked-accounts", response_model=LinkedAccountRead, status_code=201
)
...
def update_club(
    ...
    if "telegram_user_id" in data:
        ...
        if new_tid != club.telegram_user_id:
            ...
            link = (
                db.query(ClubLinkedAccount).filter_by(telegram_user_id=new_tid).first()
            )
            if link:
