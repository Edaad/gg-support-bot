...
def fetch_vaughn_cashout_rails(
    session: Session,
    audit_dates: list[date],
) -> list[CashoutRailRow]:
    """Money-sent rows whose sender name contains Vaughn, for ClubGTO audit days."""
    ...
    for audit_date in audit_dates:
        ...
        rows = (
            ...
            .filter(
                StaffCashoutMoneySend.sender_name.ilike(f"%{_VAUGHN_SENDER_NEEDLE}%"),
        ...
        for send, record in rows:
            if _VAUGHN_SENDER_NEEDLE not in (send.sender_name or "").casefold():
                continue
            ...
            out.append(
                CashoutRailRow(
                    audit_date=audit_date,
                    occurred_at=_clubgto_excel_time(send.created_at),
...
def build_gto_weekly_audit_workbook(
    ...
    *,
    ...
    cashout_rails = fetch_vaughn_cashout_rails(session, expected)
