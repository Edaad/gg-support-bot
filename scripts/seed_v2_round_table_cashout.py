#!/usr/bin/env python3
"""Seed Round Table cashout methods into greenfield club_payment_* tables.
...
Usage (dry run — no writes):
    python scripts/seed_v2_round_table_cashout.py

Apply + verify via /api/v2:
    python scripts/seed_v2_round_table_cashout.py --apply
"""
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Round Table cashout (v2): club_id={club.id}, "
