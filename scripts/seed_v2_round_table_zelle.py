#!/usr/bin/env python3
"""Seed Round Table deposit Zelle into greenfield club_payment_* tables.
...
Usage (dry run — no writes):
    python scripts/seed_v2_round_table_zelle.py

Apply + verify via /api/v2:
    python scripts/seed_v2_round_table_zelle.py --apply
"""
...
ACCUMULATED_AMOUNT = Decimal("41515.50")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Round Table Zelle (v2): club_id={club.id}, method_id={method.id}, "
            ...
            print(f"  tier: {DEFAULT_TIER_LABEL!r} (min=$50, Stripe defaults only)")
            print(f"  variant: {DEFAULT_TIER_LABEL!r} (Zelle instructions)")
            print("  sub-options: 0")
