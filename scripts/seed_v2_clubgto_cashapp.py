#!/usr/bin/env python3
"""Seed ClubGTO deposit Cashapp into greenfield club_payment_* tables.
...
variants. Does not seed the legacy hardcoded buy.stripe.com tier message.

Usage (dry run — no writes):
    python scripts/seed_v2_clubgto_cashapp.py

Apply + verify via /api/v2:
    python scripts/seed_v2_clubgto_cashapp.py --apply
"""
...
EXCLUDED_BUY_LINK = "buy.stripe.com/00weV54Y7byMagj9vwbMQ08"

UNDER_DEFAULT_RESPONSE_TEXT = """🚨 NO CREDIT CARDS. They will be refunded immediately
...
def upsert_method(session: Session, club_id: int) -> ClubPaymentMethod:
    ...
    method.max_amount = Decimal("2000")
...
def verify_via_api(club_id: int) -> None:
    ...
    if account_v.get("use_group_checkout_link") is not False:
        raise SystemExit(
            f"Expected use_group_checkout_link=false on Cashapp Account 1, "
            f"got {account_v.get('use_group_checkout_link')!r}"
        )
    if "cash.app/$eduardok4444" not in (account_v.get("response_text") or ""):
        raise SystemExit("Expected cash.app/$eduardok4444 in Cashapp Account 1 variant")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            ...
            print(
                f"Seeded ClubGTO Cashapp (v2): club_id={club.id}, method_id={method.id}, "
            ...
            print(
                ...
                "(weights 100/180/25), no buy-link variant, 0 sub-options."
            )
        else:
            session.rollback()
