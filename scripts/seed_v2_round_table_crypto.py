#!/usr/bin/env python3
"""Seed Round Table deposit Crypto into greenfield club_payment_* tables.

Idempotent upserts by (club_id, direction, slug), (method_id, tier label),
and (method_id, sub-option slug). Does not touch legacy payment_* tables.

Usage (dry run — no writes):
    python scripts/seed_v2_round_table_crypto.py

Apply + verify via /api/v2:
    python scripts/seed_v2_round_table_crypto.py --apply
"""
...
ACCUMULATED_AMOUNT = Decimal("17787.98")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Round Table Crypto (v2): club_id={club.id}, method_id={method.id}, "
