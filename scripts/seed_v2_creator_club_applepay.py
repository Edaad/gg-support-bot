#!/usr/bin/env python3
"""Seed Creator Club deposit Apple Pay into greenfield club_payment_* tables.
...
Usage (dry run — no writes):
    python scripts/seed_v2_creator_club_applepay.py

Apply + verify via /api/v2:
    python scripts/seed_v2_creator_club_applepay.py --apply
"""
...
ACCUMULATED_AMOUNT = Decimal("16634.00")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Creator Club Apple Pay (v2): club_id={club.id}, method_id={method.id}, "
