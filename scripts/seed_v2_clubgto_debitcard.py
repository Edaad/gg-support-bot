#!/usr/bin/env python3
"""Seed ClubGTO deposit Debit Card into greenfield club_payment_* tables.
...
Usage (dry run — no writes):
    python scripts/seed_v2_clubgto_debitcard.py

Apply + verify via /api/v2:
    python scripts/seed_v2_clubgto_debitcard.py --apply
"""
...
def seed(
    ...
    variant = upsert_default_variant(
        session, tier, response_text=DEBITCARD_RESPONSE_TEXT
    )
    variant.weight = 100
    session.flush()
...
def verify_via_api(club_id: int) -> None:
    ...
    if method.get("sort_order") != 3:
        raise SystemExit(f"Expected sort_order 3, got {method.get('sort_order')!r}")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded ClubGTO Debit Card (v2): club_id={club.id}, method_id={method.id}, "
            ...
            print(
                ...
                f"accumulated=null, sort_order=3)"
            )
