#!/usr/bin/env python3
"""Seed Round Table deposit Apple Pay into greenfield club_payment_* tables.

Idempotent upserts by (club_id, direction, slug) and (method_id, tier label).
Legacy had two duplicate Default tiers; V2 stores exactly one Default tier.

Usage (dry run — no writes):
    python scripts/seed_v2_round_table_applepay.py

Apply + verify via /api/v2:
    python scripts/seed_v2_round_table_applepay.py --apply
"""
...
ACCUMULATED_AMOUNT = Decimal("20561.00")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Round Table Apple Pay (v2): club_id={club.id}, method_id={method.id}, "
