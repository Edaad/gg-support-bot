#!/usr/bin/env python3
"""Seed Creator Club cashout methods into greenfield club_payment_* tables.

Idempotent upserts for Crypto (11 sub-options), Cashapp, Zelle, and Venmo.
Player copy on sub-options (crypto) or Default variants (text methods).

Usage (dry run — no writes):
    python scripts/seed_v2_creator_club_cashout.py

Apply + verify via /api/v2:
    python scripts/seed_v2_creator_club_cashout.py --apply
"""
...
def _verify_crypto_method(method: dict) -> None:
    ...
    if min_amount is None or Decimal(str(min_amount)) != MIN_AMOUNT:
        raise SystemExit(f"Expected crypto min_amount={MIN_AMOUNT}, got {min_amount!r}")
    ...
    if len(subs) != 11:
        raise SystemExit(f"Expected 11 crypto sub-options, got {len(subs)}")
...
def _verify_text_method(method: dict, slug: str) -> None:
    ...
    min_amount = method.get("min_amount")
    if min_amount is None or Decimal(str(min_amount)) != MIN_AMOUNT:
        raise SystemExit(f"Expected {slug} min_amount={MIN_AMOUNT}, got {min_amount!r}")
