#!/usr/bin/env python3
"""Seed Creator Club deposit Debit Card into greenfield club_payment_* tables.
...
Usage (dry run — no writes):
    python scripts/seed_v2_creator_club_debitcard.py

Apply + verify via /api/v2:
    python scripts/seed_v2_creator_club_debitcard.py --apply
"""
...
ACCUMULATED_AMOUNT = Decimal("2190.00")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded Creator Club Debit Card (v2): club_id={club.id}, method_id={method.id}, "
