#!/usr/bin/env python3
"""Seed ClubGTO deposit Apple Pay into greenfield club_payment_* tables.

Idempotent upserts by (club_id, direction, slug) and (method_id, tier label).
Legacy stored checkout copy on the method row; V2 moves it to a Default variant
with Stripe defaults on the tier.

Usage (dry run — no writes):
    python scripts/seed_v2_clubgto_applepay.py

Apply + verify via /api/v2:
    python scripts/seed_v2_clubgto_applepay.py --apply
"""
...
CLUB_NAME = "ClubGTO"
METHOD_SLUG = "applepay"
DIRECTION = "deposit"
DEFAULT_TIER_LABEL = "Default"

APPLEPAY_RESPONSE_TEXT = """🚨 NO CREDIT CARDS. They will be refunded immediately
...
def verify_via_api(club_id: int) -> None:
    ...
    methods = resp.json()
    applepay = [m for m in methods if m.get("slug") == METHOD_SLUG]
    if len(applepay) != 1:
        raise SystemExit(f"Expected exactly one applepay method, got {len(applepay)}")

    method = applepay[0]
    if method.get("has_sub_options"):
        raise SystemExit("Expected has_sub_options=false on Apple Pay method")
    ...
    if method.get("sort_order") != 2:
        raise SystemExit(f"Expected sort_order 2, got {method.get('sort_order')!r}")
    ...
    text = variants[0].get("response_text") or ""
    if "{{hyperlink}}" not in text:
        raise SystemExit("Expected {{hyperlink}} placeholder in variant response_text")
    if "NO CREDIT CARDS" not in text:
        raise SystemExit("Expected NO CREDIT CARDS in variant response_text")
...
def main() -> None:
    ...
    try:
        ...
        if args.apply:
            session.commit()
            print(
                f"Seeded ClubGTO Apple Pay (v2): club_id={club.id}, method_id={method.id}, "
            ...
            print(
                f"  method: Apple Pay / {METHOD_SLUG} (deposit, $20–$100, "
                f"accumulated=null, sort_order=2)"
            )
