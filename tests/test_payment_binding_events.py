...
class TestPaymentBindingEvents(IsolatedAsyncioTestCase):
    ...
    def test_track_ingest_notification_unbound(self, mock_record):
        ...
        self.assertEqual(
            mock_record.call_args[0][0].event_type, EVENT_NOTIFICATION_SENT
        )
    ...
    async def test_sync_payment_notification_edit_ok(
        self, mock_edit, mock_record, _mock_list
    ):
    ...
    @patch("bot.services.payment_binding_events.record_binding_event")
    async def test_sync_payment_notification_edit_skipped(
        self, mock_record, _mock_list
    ):
        ok = await sync_payment_notification_edit(
            payment_method_slug="zelle",
            payment_id=81,
    ...
    async def test_sync_payment_notification_edit_failed(
        self, mock_edit, mock_record, _mock_list
    ):
