...
class CashoutRecordsApiTestCase(unittest.TestCase):
    ...
    def test_list_audited_admin_ok(self) -> None:
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(resp.json()["items"][0]["audited"])
        self.assertEqual(mock_list.call_args.kwargs["audited"], True)

    def test_list_audited_am_forbidden(self) -> None:
        app = _make_api_app()
        app.dependency_overrides[get_current_admin] = lambda: ROLE_ACCOUNT_MANAGER
        client = TestClient(app)
        resp = client.get("/api/cashout-records?audited=true")
    ...
    def test_patch_audited_admin_ok(self) -> None:
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(resp.json()["audited"])
        self.assertEqual(mock_update.call_args.kwargs["audited"], True)

    def test_patch_audited_am_forbidden(self) -> None:
        app = _make_api_app()
        app.dependency_overrides[get_current_admin] = lambda: ROLE_ACCOUNT_MANAGER
        client = TestClient(app)
        resp = client.patch("/api/cashout-records/1", json={"audited": True})
    ...
    def test_patch_audited_gto_forbidden(self) -> None:
        from api.auth import ROLE_GTO

        app = _make_api_app()
        app.dependency_overrides[get_current_admin] = lambda: ROLE_GTO
        with patch(
