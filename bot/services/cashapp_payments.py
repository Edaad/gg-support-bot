...
from db.models import CashAppPayment
...
def format_setup_already_linked_warning(
    ...
    *,
    ...
    lines = format_setup_blocked_header_lines(
        single_title_preamble="⚠️ First-time Cash App setup warning",
    ...
    *,
    ...
    with get_db() as session:
        ...
        if not auto_bound and not setup_blocked_already_linked:
            ...
            ambiguous_candidates = candidates_for_payment(session, payment, "cashapp")
    ...
    if setup_blocked_already_linked and setup_target_chat_id and setup_target_title:
        ...
        with get_db() as session:
            existing_ids = candidate_chat_ids(
                session, "cashapp", payer_name=payment.payer_name
            )
    ...
    log_ingest_bind_delivery(
        method_slug="cashapp",
        payment_id=payment_id,
        identity_label=identity_label("cashapp", payer_name=payment.payer_name),
...
async def bind_cashapp_payment_from_reply(
    *,
    ...
    if ref is None or ref.method_slug != "cashapp":
