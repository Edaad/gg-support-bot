...
def resolve_deposit_session_for_chat(
    telegram_chat_id: int,
    *,
    within_minutes: int = CORRELATION_WINDOW_MINUTES,
) -> DepositFunnelEvent | None:
    """Latest wizard session for a chat (instructions_sent or deposit_started)."""
    since = _utc_now() - timedelta(minutes=int(within_minutes))
    with get_db() as session:
        for step in (STEP_INSTRUCTIONS_SENT, STEP_DEPOSIT_STARTED):
            row = (
                session.query(DepositFunnelEvent)
                .filter(
                    DepositFunnelEvent.telegram_chat_id == int(telegram_chat_id),
                    DepositFunnelEvent.step == step,
                    DepositFunnelEvent.created_at >= since,
                )
                .order_by(DepositFunnelEvent.created_at.desc())
                .first()
            )
            if row is not None:
                return row
...
def _session_fields_from_anchor(
    anchor: DepositFunnelEvent,
) -> dict[str, Any]:
    return {
        "deposit_session_id": str(anchor.deposit_session_id),
        "club_id": int(anchor.club_id) if anchor.club_id is not None else None,
        "telegram_user_id": (
            int(anchor.telegram_user_id)
            if anchor.telegram_user_id is not None
            else None
        ),
        "is_first_deposit": bool(anchor.is_first_deposit),
        "requires_method_setup": bool(anchor.requires_method_setup),
    }
...
def record_payment_funnel_from_ingest(
    *,
    ...
    if telegram_chat_id is None:
        return
    anchor = resolve_deposit_session_for_chat(int(telegram_chat_id))
    if anchor is None:
        return
    base = _session_fields_from_anchor(anchor)
    meta = {
...
def record_chips_credited_funnel(
    *,
    ...
    """Record chips_credited (+ chips_confirmed) for a correlated deposit session."""
    anchor = resolve_deposit_session_for_chat(int(telegram_chat_id))
    if anchor is None:
        return
    base = _session_fields_from_anchor(anchor)
