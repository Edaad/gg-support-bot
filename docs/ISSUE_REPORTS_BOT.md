...
The wizard is keyed per chat + user (`per_chat=True`), so steps only continue in the DM where `/report` started — not in support groups.

### System-generated reports

Venmo and Zelle ingest auto-create issue reports when a payment requires a refund: **Goods & Services** (Venmo) or a **banned memo** (gambling/poker/club-name keywords). Non-whole-dollar amounts do not open a ticket. These tickets notify **Head admin** with a lean Slack body (reason + handle + memo; no group/GG id). Visible in `/reports` like any other deposit report. No staff action is needed to open the ticket. Reporter source is `venmo_ingest` or `zelle_ingest`.
