...
**Escalation notification (player idle / cashout / deposit chase):** Club dashboard toggle **Escalation notification**. Slack uses a **dedicated** channel (`SLACK_ESCALATION_BOT_TOKEN` + `SLACK_ESCALATION_CHANNEL_ID`, or `SLACK_ESCALATION_WEBHOOK_URL`). See [`docs/ESCALATION_NOTIFICATION.md`](ESCALATION_NOTIFICATION.md). Run once:
