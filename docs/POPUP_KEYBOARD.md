...
Idle install may omit **`/cashout`** when cashout would be denied by the group’s cooldown timer (read-only check; one-time bypass is ignored for the button). Business hours are advisory only and do not hide the button. Groups with a **permanent** cashout bypass always get both buttons. Typed `/cashout` is unchanged.
