"""Add per-tier Stripe group-checkout settings (same as payment_methods).

Adds to payment_method_tiers:
...
def main() -> None:
    with get_db() as session:
        if not _has_column(session, "payment_method_tiers", "use_group_checkout_link"):
            session.execute(
                text(
                    """
                    ALTER TABLE payment_method_tiers
                    ADD COLUMN use_group_checkout_link BOOLEAN NOT NULL DEFAULT FALSE
                    """
                )
            )

        if not _has_column(session, "payment_method_tiers", "group_checkout_provider"):
            session.execute(
                text(
                    """
                    ALTER TABLE payment_method_tiers
        ...
        if not _has_column(session, "payment_method_tiers", "hyperlink_text"):
            session.execute(
                text(
                    """
                    ALTER TABLE payment_method_tiers
